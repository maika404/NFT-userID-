...
contract DubID is ERC721URIStorage {
  ...
  mapping(string => bytes32) public TxId;
  mapping (uint => string) public names;


  constructor(string memory _tld) payable ERC721("Browser Name Service", "NNS") {
...
// This function will give us the price of a domain based on length
  ...
  function register(string calldata name) public payable {
    ...
    string memory json = Base64.encode(
      string(abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on the Browser name service", "image": "data:image/svg+xml;base64,',
  ...
  function setRecord(string calldata name) public {
      // Check that the owner is the transaction sender
      if (msg.sender != domains[name]) revert Unauthorized();
      bytes memory CounterData = abi.encodePacked(_tokenIds.current());
      TxId[name] = keccak256(CounterData);
  }

  function getRecord(string calldata name) public view returns(bytes32) {
      return TxId[name];
  }
